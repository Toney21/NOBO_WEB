...
const OrderDetails = ({ OrderIdDigital }) => {
    ...
    const resolvePaymentMethodText = (orderData) => {
        if (!orderData) return ''

        if (orderData?.payment_method_label) {
            return orderData?.payment_method_label
        }

        if (
            orderData?.offline_payment !== null &&
            orderData?.payment_method !== 'partial_payment'
        ) {
            return `${t('Offline Payment')} (${orderData?.offline_payment?.data?.method_name})`
        }

        return `${t(orderData?.payment_method).replaceAll('_', ' ')}`
    }
    const resolvePaymentStatusText = (orderData) => {
        if (!orderData) return ''
        if (orderData?.payment_status_label) {
            return orderData?.payment_status_label
        }
        return orderData?.payment_status === 'paid' ? t('Paid') : t('Unpaid')
    }
    const resolvePaymentStatusColor = (orderData) => {
        if (!orderData) return theme.palette.neutral[400]
        if (
            orderData?.payment_method === 'corporate_account' &&
                                                        ...
                                                        trackData?.data
                                                            ...
                                                            <Typography
                                                                sx={{
                                                                    ...
                                                                    fontSize:
                                                                        '14px',
                                                                    color: resolvePaymentStatusColor(
                                                                        trackData?.data
                                                                    ),
                                                                }}
                                                                align="left"
                                                            >
                                                                {resolvePaymentStatusText(
                                                                    trackData?.data
                                                                )}
                                                            </Typography>
                                                        )}
