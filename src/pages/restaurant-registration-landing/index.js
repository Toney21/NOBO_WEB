...
export const getServerSideProps = async (context) => {
    ...
    const pathName = resolveRequestOrigin(req) + resolvedUrl
    const baseUrl = normalizeBackendBaseUrl(process.env.NEXT_PUBLIC_BASE_URL)
    const origin = resolveRequestOrigin(req, process.env.NEXT_CLIENT_HOST_URL)
    ...
    try {
        const landingPageRes = await fetch(
            `${baseUrl}/api/v1/react-registration-page`,
            {
                method: 'GET',
                headers: {
                    'X-software-id': 33571750,
    ...
    metaData = await fetchPageMetadata('restaurant_join_page', null, language, req)
