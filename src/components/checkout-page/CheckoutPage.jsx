...
const tomorrow = nextday.format('dddd')
...
const CheckoutPage = ({ isDineIn }) => {
    ...
    useQuery(['checkout-config', token], ConfigApi.config, {
        enabled: !!token,
        onSuccess: (res) => {
            if (res?.data) {
    ...
    const handleOrderMutationObject = (carts, productList) => {
        ...
        const selectedPaymentMethod =
            paymenMethod === 'corporate_account'
                ? 'corporate_account'
    ...
    const handlePlaceOrder = () => {
        ...
        if (isAvailable) {
            ...
            } else if (paymenMethod === 'corporate_account') {
                let totalQty = 0
                let carts = handleProductList(productList, totalQty)
                const handleSuccessCorporate = (response) => {
                    setOrderId(response?.data?.order_id)
                    toast.success(
                        response?.data?.message ||
                            t('Corporate order placed successfully.')
                    )
                    setOrderSuccess(true)
                }
                if (carts?.length > 0) {
    ...
    const activateCorporateCheckout = () => {
        const paymentLabel =
            corporateCheckout?.payment_method_label || 'Corporate Account'
        const statusLabel =
            corporateCheckout?.payment_status_label || 'Corporate Billing'

        setPaymenMethod('corporate_account')
        setSelected({
    ...
    const hasOnlyPaymentMethod = () => {
        if (isCorporateCheckoutLocked) {
            activateCorporateCheckout()
            return
        }
        ...
        if (
            ...
            if (global?.cash_on_delivery) {
                ...
                if (!paymenMethod || paymenMethod === 'corporate_account') {
