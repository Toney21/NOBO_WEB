...
const ANALYTICS_ENDPOINT = "/api/v1/config/get-analytic-scripts";

const resolveBaseUrl = (ctx) => {
  const envBaseUrl = normalizeBackendBaseUrl(process.env.NEXT_PUBLIC_BASE_URL);
  if (envBaseUrl) {
    return envBaseUrl;
  }
...
CustomDocument.getInitialProps = async (ctx) => {
  ...
  try {
    ...
    if (baseUrl) {
      ...
      let res = null;
      try {
        res = await fetch(`${baseUrl}${ANALYTICS_ENDPOINT}`, {
          headers: {
            "X-software-id": 33571750,
            "X-server": "server",
            origin: resolveRequestOrigin(ctx?.req, process.env.NEXT_CLIENT_HOST_URL || baseUrl),
          },
